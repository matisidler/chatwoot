...
RSpec.describe AgentNotifications::ConversationNotificationsMailer do
  ...
  describe 'conversation_creation' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, Se ha creado una conversación (##{conversation.display_id}) en #{conversation.inbox&.name}.")
    end
  ...
  describe 'conversation_assignment' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, Se te ha asignado una conversación (##{conversation.display_id}).")
    end
  ...
  describe 'conversation_mention' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, Has sido mencionado en la conversación (##{conversation.display_id}).")
    end
    ...
    it 'renders the senders name' do
      expect(mail.body.encoded).to match("Has sido mencionado en una conversación. <b>#{another_agent.display_name}</b> escribió:")
    end
  ...
  describe 'assigned_conversation_new_message' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, Se ha creado un nuevo mensaje en tu conversación asignada (##{message.conversation.display_id}).")
    end
  ...
  describe 'participating_conversation_new_message' do
    ...
    it 'renders the subject' do
      expect(mail.subject).to eq("#{agent.available_name}, Se ha creado un nuevo mensaje en tu conversación participante (##{message.conversation.display_id}).")
    end
